...
import { fetchUserData, changePassword, forgotPassword } from "@site/src/api";
...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error(translate({ id: 'error.fetch', message: 'Error fetching user data:'}), error);
        window.location.href = "/"; // If no auth_token, redirect to homepage
      }
    };
  ...
  const onFinishChangePassword = async (values) => {
    ...
    try {
      ...
      console.error(translate({ id: 'error.changePassword', message: 'Error changing password:'}), error);
  ...
  const items = [
    {
      ...
      children: (
        <Form onFinish={onFinishChangePassword}>
          ...
          >
            <Input.Password placeholder={translate({ id: 'placeholder.currentPassword', message: '当前密码'})} />
          </Form.Item>
          <Form.Item
            name="newPassword"
  ...
  return (
    <Layout>
      <div style={{ width: 600, margin: "auto", padding: "50px 0" }}>
        <Link to="/"><Translate id="link.home">返回首页</Translate></Link>
        <Card title={translate({ id: 'title.userInfo', message: '用户信息'})}>
          <p><Translate id="userInfo.username">用户名：</Translate> {userData.username}</p>
          <p><Translate id="userInfo.email">邮箱：</Translate> {userData.email}</p>
        </Card>
