...
import { updateUsername, clearAll } from "@site/src/api";
...
const UserProfile = () => {
  ...
  /**
   * 【离线版】这里清的是**用户数据本身**，不是缓存。
   *
   * 在线版「清除缓存」是安全操作：清掉本地副本后从 /myspace 重新拉。离线版没有服务端，
   * 那四个 localStorage key 就是唯一的一份数据，清掉即永久删除。
   * 沿用在线版的 clearUserProfileCache/clearMySpaceCache 在这条线上是空操作 ——
   * 会出现「点了清除、提示成功、实际什么都没发生」的假成功，所以必须直接清本地存储。
   *
   * 注意：按钮与确认框的文案仍是在线版的「清除缓存 / 清除后将重新加载所有数据」，
   * 与离线版的实际语义（不可恢复的删除）不符，需要单独改文案 —— 见分支说明。
   */
  const handleClearCache = useCallback(async () => {
    try {
      ...
      await fetchUserInfo();
      // 同 DataManagementCard：硬编码英文，不用「缓存已清除」那套译文——离线版这里删的是唯一一份数据。
      message.success("Local data deleted");
    } catch (error) {
      console.error("Clear cache error:", error);
      message.error("Couldn't delete local data. Try again.");
    }
          ...
          <Col xs={24} sm={22} md={20} lg={16} xl={14} className="full-width-col">
            <Space orientation="vertical" size="large" style={{ width: "100%" }}>
              ...
              <Row gutter={[24, 24]}>
                ...
                {/* 离线版没有账号体系，因此没有「安全设置」列（在线版这里是 SecurityCard：
                    改密 / 忘记密码 / 邮箱）。左列因此独占整行。 */}
              </Row>
