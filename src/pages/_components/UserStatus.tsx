  ...
  if (userAuth && userAuth.data) {
    return (
      <div>
        ...
        <Modal title={translate({ id: "modal.addprompt.title", message: "分享 Prompt（本内容将出现在「你的提示词」标签中）" })} open={open} footer={null} onCancel={() => setOpen(false)}>
          <Form onFinish={onFinish}>
            <Form.Item name='title' rules={[{ required: true, message: translate({ id: "message.addprompt.requiredTitle", message: "请输入提示词标题！" }) }]}>
              <Input placeholder={translate({ id: "input.addprompt.title", message: "提示词名称" })} />
            </Form.Item>
            <Form.Item name='description' rules={[{ required: true, message: translate({ id: "message.addprompt.requiredDescription", message: "请输入提示词内容！" }) }]}>
