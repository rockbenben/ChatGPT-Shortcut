...
![离线版数据架构：一台内网服务器托管共享只读的提示词库，每位用户的收藏、提示词、排序和标签各自存在浏览器 localStorage，互不影响，无需账户](/img/docs/zh/offline-architecture.svg)
...
进入「我的账户 → 数据管理 → 导出数据」，将个人收藏和自建提示词导出为 JSON 文件。
...
可以。一人导出 JSON 文件后，其他成员在「我的账户 → 数据管理 → 导入数据」中导入即可，自动去重。
