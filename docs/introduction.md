...
AiShort 还提供了一个浏览器扩展（ChatGPT Shortcut），支持 Chrome、Edge、Firefox 以及其他基于 Chromium 的浏览器。该扩展不仅继承了网页版的全部功能，还添加了如侧边栏和自动激活窗口等特性。该扩展能够随 ChatGPT 或自定义页面自动启动，同时支持通过 `Alt+Shift+S` 快捷键手动激活。以下为下载渠道：
...
此外，我们还提供了油猴脚本——[**ChatGPT Shortcut Anywhere**](https://greasyfork.org/scripts/482907-chatgpt-shortcut-anywhere)，该脚本允许你自定义匹配域名，在任意网站上使用 AiShort 侧边栏。不过，由于 ChatGPT 页面对脚本内容注入的限制，侧边栏功能在该页面上将通过弹窗形式激活。
