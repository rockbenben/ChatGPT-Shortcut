...
```
خادم الشبكة الداخلية (النسخة دون اتصال منشورة عليه)
   ├── بيانات مكتبة المطالبات (مشتركة للجميع، من JSON ثابت)
   │
   ├── متصفح المستخدم A → localStorage (مفضلات A ومطالباته الخاصة)
   ├── متصفح المستخدم B → localStorage (مفضلات B ومطالباته الخاصة)
   └── متصفح المستخدم C → localStorage (مفضلات C ومطالباته الخاصة)
```
...
- مسح بيانات المتصفح يؤدي إلى فقدان البيانات الشخصية — يُوصى بعمل نسخ احتياطية دورية عبر «الإعدادات > تصدير البيانات».
...
ادخل إلى «الإعدادات > تصدير البيانات»، وصدِّر مفضلاتك الشخصية ومطالباتك المخصصة إلى ملف JSON.
...
2. استورد ملف JSON من صفحة «الإعدادات» في النسخة دون اتصال
...
يُوصى بعمل نسخ احتياطية دورية للبيانات المهمة في ملف JSON عبر «الإعدادات > تصدير البيانات».

### هل يمكن مشاركة المطالبات المخصصة بين أعضاء الفريق؟

نعم. بعد أن يُصدِّر أحدهم ملف JSON، يستوردُه الآخرون من «الإعدادات > استيراد البيانات»، ويتم إزالة التكرار تلقائيًا.
