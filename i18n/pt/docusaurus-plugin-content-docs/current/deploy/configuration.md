...
├── user.ts        # user info
├── homepage.ts    # dados dos cartões da página inicial (apenas JSON estático, sem backend)
└── sessionCache.ts # rastros locais a limpar no logout (3 caminhos compartilham)
```
