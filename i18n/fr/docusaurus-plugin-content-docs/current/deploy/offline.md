...
![Architecture des données de la version hors ligne : un serveur intranet héberge une bibliothèque de prompts partagée en lecture seule ; les favoris, prompts, ordre d'affichage et balises de chaque utilisateur résident dans le localStorage de son propre navigateur — indépendants, sans compte.](/img/docs/offline-architecture.svg)
...
- Effacer les données du navigateur entraîne la perte des données personnelles — il est recommandé de sauvegarder régulièrement via «&nbsp;Mon Compte → Gestion des Données → Exporter les données&nbsp;».
...
Rendez-vous dans «&nbsp;Mon Compte → Gestion des Données → Exporter les données&nbsp;» pour exporter vos favoris personnels et vos prompts personnalisés sous forme de fichier JSON.
...
2. Sur la version hors ligne, importez ce fichier JSON depuis «&nbsp;Mon Compte → Gestion des Données&nbsp;»
...
Il est recommandé de sauvegarder régulièrement les données importantes au format JSON via «&nbsp;Mon Compte → Gestion des Données → Exporter les données&nbsp;».

### Peut-on partager des prompts personnalisés au sein de l'équipe&nbsp;?

Oui. Un utilisateur exporte son fichier JSON, les autres membres l'importent via «&nbsp;Mon Compte → Gestion des Données → Importer les données&nbsp;»&nbsp;; la déduplication est automatique.
