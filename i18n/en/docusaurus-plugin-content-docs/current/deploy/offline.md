...
![Offline edition data architecture: one intranet server hosts a shared, read-only prompt library; each user's favorites, prompts, order and tags live in their own browser localStorage — independent, no account](/img/docs/offline-architecture.svg)
