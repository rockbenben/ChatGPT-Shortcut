...
## オフライン版デプロイ（企業イントラネット）

企業イントラネット、政府ネットワークなど外部ネットワークにアクセスできない環境では、`offline` ブランチを使用できます。バックエンドサーバーは不要で、データはブラウザにローカル保存されます。Vercel/Cloudflare Pages でデプロイする場合は `offline` ブランチを選択してください。Docker デプロイ：

```bash
docker run -d -p 3000:3000 --name aishort-offline ghcr.io/rockbenben/chatgpt-shortcut:offline
```

詳細については[オフライン版ドキュメント](./guides/offline)をご覧ください。
...
# `scripts/i18nLocales.mjs` ファイルの `defaultLocale` を更新し、希望する言語のビルドを実行します。
