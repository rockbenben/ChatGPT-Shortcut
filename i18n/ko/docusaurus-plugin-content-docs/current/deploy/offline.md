...
![오프라인 버전 데이터 아키텍처: 사내 서버 한 대가 공유 읽기 전용 프롬프트 라이브러리를 호스팅하고, 각 사용자의 즐겨찾기·프롬프트·정렬·태그는 본인 브라우저의 localStorage에 저장되어 서로 독립적이며 계정이 필요 없습니다](/img/docs/offline-architecture.svg)
...
2. 오프라인 버전의 「내 계정 → 데이터 관리 → 가져오기」에서 해당 JSON 파일을 가져옵니다
