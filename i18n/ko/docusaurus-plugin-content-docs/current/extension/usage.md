---
sidebar_label: 확장 프로그램 사용법
title: AI Short 확장 프로그램 가이드 - 사이드바 및 단축키
description: AI Short 브라우저 확장 프로그램 마스터! Alt+Shift+S로 프롬프트 라이브러리 호출.
---

# 브라우저 확장 프로그램 사용법

ChatGPT Shortcut은 18개 주요 언어를 지원합니다. 확장 프로그램 언어는 브라우저 환경에 따라 자동 설정됩니다. 설정 페이지에서 수동으로 변경할 수도 있습니다.

![](/img/docs/extension-settings.png)

## 표시 모드

확장 프로그램은 세 가지 표시 모드를 지원하며 사용 습관에 따라 선택할 수 있습니다.

- **사이드바 모드**: 브라우저 사이드바에서 열며 웹 콘텐츠와 나란히 표시 — AI 대화 페이지를 보면서 프롬프트를 탐색하기에 적합
- **팝업 모드**: 확장 프로그램 아이콘 클릭 시 작은 창으로 팝업 — 빠르게 확인하고 닫을 수 있어 화면을 차지하지 않음
- **창 모드**: 독립적인 플로팅 창에서 열기 — 보조 모니터로 드래그하거나 독립적으로 배치 가능

## 사이드 패널 열어두기

이 옵션을 활성화하면 브라우저 탭을 전환해도 사이드바가 열린 상태로 유지되어 지속적인 사용에 편리합니다.

## 다크 모드

확장 프로그램은 다크 모드를 지원합니다. 설정 페이지에서 "다크 모드" 옵션을 활성화하면 인터페이스가 다크 모드로 전환됩니다.

![](/img/docs/extension-sidebar.webp)

## 단축키 활성화

`Alt+Shift+S` 단축키로 확장 프로그램을 직접 활성화할 수 있습니다.
