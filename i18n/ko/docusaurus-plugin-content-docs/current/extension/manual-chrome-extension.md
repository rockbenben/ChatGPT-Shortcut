...
> 본 가이드는 [설치 페이지](./README.md)에서 이미 CRX 설치 패키지를 다운로드하고 압축을 푼 상태를 전제로 합니다 (.crx 파일은 압축을 푼 폴더 안에 있습니다).
>
> ⚠️ **자주 발생하는 두 가지 실수**:
> 1. 반드시 압축을 푼 **`.crx` 파일**을 확장 프로그램 페이지로 드래그해야 합니다. zip 설치 패키지 자체를 드래그하면 안 됩니다
> 2. 반드시 **드래그**해야 합니다. "압축해제된 확장 프로그램을 로드합니다" 버튼은 클릭하지 마세요 (그것은 zip 버전용입니다)
...
4. 브라우저가 crx 파일 설치를 허용하지 않나요? zip 파일 설치를 시도해 보세요! [zip 설치 튜토리얼을 보려면 여기를 클릭하세요](./manual-chrome-extension-zip).
