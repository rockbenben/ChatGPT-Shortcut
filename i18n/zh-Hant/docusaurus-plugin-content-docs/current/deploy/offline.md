...
![離線版資料架構：一台內網伺服器託管共享唯讀的提示詞庫，每位使用者的收藏、提示詞、排序和標籤各自存在瀏覽器 localStorage，互不影響，無需帳戶](/img/docs/zh/offline-architecture.svg)
...
- 清除瀏覽器數據會丟失個人數據——建議定期通過「我的帳戶 → 資料管理 → 導出數據」備份。
- 更換電腦或瀏覽器後需重新導入數據。
...
進入「我的帳戶 → 資料管理 → 導出數據」，將個人收藏和自建提示詞導出爲 JSON 文件。
...
建議重要數據定期通過「我的帳戶 → 資料管理 → 導出數據」備份爲 JSON 文件。

### 能否在團隊間共享自建提示詞？

可以。一人導出 JSON 文件後，其他成員在「我的帳戶 → 資料管理 → 導入數據」中導入即可，自動去重。
